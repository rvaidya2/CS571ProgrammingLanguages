...
dept_employees1(Dept, [Employees|Tail]) ->
if
  Employees#employee.dept == Dept -> [Employees|dept_employees1(Dept, Tail)];
...
% [E] appended.  
upsert_employee(E, [Employees|Tail]) -> upsert_employee(E, [Employees|Tail], 0).
upsert_employee(E, [], 0) -> [E];
upsert_employee(E, [], 1) -> [];
upsert_employee(E, [Employees|Tail], Acc) ->
  if
  Employees#employee.name == E#employee.name -> [E|upsert_employee(E, Tail, 1)];
  true -> [Employees|upsert_employee(E, Tail, Acc)]
end.
...
find_employees(Preds, Employees) -> [X|| X<-Employees, lists:all(fun(Y) ->Y(X) end,Preds)].
...
employees_req(Req, Employees) -> 
  case (Req) of
    ...
    { read, Name } ->
      S =find_employees([employee_has_name(Name)], Employees),
      if(S == []) ->
      {err, not_found ,Employees};
      true ->
        [H] = S,
        {ok,H, Employees}
      end;
...
employees_server(Employees, Fn) ->
  receive
    {Pid, Request} -> case Request of
        {stop} -> Pid ! {self(), {ok, stopped}};
        {new_fn, Fn1} -> Pid ! {self(),{ok,void}}, employees_server(Employees, Fn1);
        _ -> { Status, Result, Employees1 } = Fn(Request, Employees), Pid ! { self(), { Status, Result }}, employees_server(Employees1, Fn)
      end
    end.


start_employees_server(Employees, Fn) -> 
  PID = spawn(prj5_sol , employees_server, [Employees, Fn]),
    register(employee, PID),
    PID.

% stop previously started server with registered ID emps.
% should return {ok, stopped}.
stop_employees_server() -> 
   employees_client(stop),
    unregister(employee).
