...
modulus(Div,X):-mod(Div,X)=:=0.
divisible_by(Numbers,Div,Result):-
    member(Result,Numbers),
    modulus(Result,Div).
...
rule_clause((C1:-C2), R):-
    C1 \= (_,_),C2 \= (_,_),R =  (C1 \/ ~C2).

rule_clause((C1:-C2,C3), R):-
    C1 \= (_,_),C2 \= (_,_), C3 \= (_,_),R =  (C1 \/ ~C2\/ ~C3).

rule_clause((C1:-C2,C3,C4), R):- 
    R =  (C1 \/ ~C2\/ ~C3 \/ ~C4 ).
